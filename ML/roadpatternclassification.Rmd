...
```{r,cache=TRUE}
angle_probs= df_Pa %>% select(-City)
entropie = -rowSums(angle_probs*log2(angle_probs))
...
```{r,cache=TRUE}
# give a bollean vector of 
intrain  = createDataPartition(Xp$Cont, list=FALSE,p = 0.75)
# learn a scaler (mean and sd estimation) to normalize the data see ?preProcess
scaler = preProcess(Xp[intrain,])
# transform and select the train set
Xp.train = predict(scaler,Xp[intrain,])
# transform and select the test set
Xp.test  = predict(scaler,Xp[-intrain,])
```
...
# reconstruct the prediction, with glm predict will return the probabilities of the 
probabilities = predict(fit,Xp.train,type="response")
# but we may convert to most probables labels manualy
classes = levels(Xp.train$Cont)
predlab.train = factor(classes[(probabilities>0.5)+1],levels = classes)
# and compute summary statistics on the train set
...
Higher value of k seems to help a little bit to increase the perfomances. You may look at the results of the crossvalidation by looking at the **fitcv$result** table by ourself which gives you also standard deviation for our generalisation performances.
We may also get a final vue of the performances on the test set of the chosen model. This code will evaluate te performance of the best model on the test set.
...
- choose the value of an hyper-parameter, with grid search and cross validation (**expand.grid, trainControl, train, predict**)
...
```{r,cache=TRUE}
# read and clean data
cities = read_sf("./shp/Cities2.shp") %>% filter(ang180!=0) %>% mutate(Cont=factor(Cont))
...
### Question 1
Fit a k-nn on the complete data-set and tune k. You may 



### Question 2

Help yourself in using caret to fit a cart tree (with **rpart**) and compute the model test accuracy. A list of available methods in caret is available [here](https://topepo.github.io/caret/train-models-by-tag.html) together with the name of their hyper-parameters that you must fix (by crossvalidation or other means). 

To get a visual explanation of this type of algorithm you may first spend some minutes with these visual tutorials [http://www.r2d3.us/visual-intro-to-machine-learning-part-1/](http://www.r2d3.us/visual-intro-to-machine-learning-part-1/) and [http://www.r2d3.us/visual-intro-to-machine-learning-part-2/](http://www.r2d3.us/visual-intro-to-machine-learning-part-2/). you may visualize the structure of your final tree with the prp function of the rpart.plot package. To get the final model you may acces the finalModel slot of the list returned by the train function of caret. You may also use the function **varImp** on this type of model to get a score of importance for each variable (look at the doc).
...
In case of training problem or if it takes too long a trained cnn is available in the project directory. It was saved using this code blocks and ban be loaded with the command in comment.
```{r,cache=TRUE}
