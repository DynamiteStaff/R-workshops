...
dfh = subset(dfb, property_group == 'home' & as.character(dfb$host_neighbourhood) == as.character(df$neighbourhood))
dim(dfh)[1] - dim(df)[1]
...
final <- left_join(df, lookup, by = c("property_type" = "type")) %>% 
  mutate(year = as.numeric(substr(last_review, 1, 4)),
         ...
         rooms = ifelse(bedrooms == 0, 1, bedrooms),
         priceperroom = as.numeric(ifelse(room_type == T,  numPrice,numPrice / rooms))) %>% 
  filter(property_group == 'home' & 
           as.character(host_neighbourhood) == as.character(neighbourhood) & 
           !duplicated(host_id) & 
           year >= 2017 & room_type != "Shared room")  
...
```{r censusDiversityFunctions, include=FALSE}
...
```{r applyCensusDiversityFunctions, message=F, warning=F, include=F}
...
```{r CensusSegregationFunctions, include=FALSE}
...
```{r airbnbSegregationFunction, include=F}
...
```{r applyCensusSegregationFunctions, include=F}
...
```{r selectCityInCensus, include=F}
...
```{r selectCityInsegregation, include = F}
