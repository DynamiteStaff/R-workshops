...
displayState <- function(meltedState, dotSize=1){
...
meltTestEnv <-  meltTestEnv %>% rowwise() %>% mutate(happy= isHappy(state = testEnv,X,Y))

# we update the display function code
displayStateUnhappy <- function(meltedState){
  pp <- displayState(meltTestEnv, dotSize = 5) + 
    geom_text(data = subset(meltTestEnv, !happy ), aes(X,Y,label = ":-|"), angle=-90)  
return(pp)
  }


# save a function for later
...
empty <-  getEmptyCells(meltTestEnv)
...
unhappy <-  getUnhappy(meltTestEnv)
numberOfEmpty <-  nrow(getEmptyCells(meltTestEnv))
cat(numberOfEmpty) # has to be 20% of size squared, every time
