...
isHappy <- function(state, i , j, tolerance){
  ...
  numberOfDiff <-sum(cell!=neigh, na.rm = T)  
  happy <-  numberOfDiff / (length(neigh) ) < tolerance
  return(happy)  
} 
```
...
testEnv <-  matrixSchellingState(10,pctOccupied = 0.8)
...
meltTestEnv <-  meltTestEnv %>% rowwise() %>% mutate(happy= isHappy(state = testEnv,X,Y,tolerance = 0.3))
...
updateHappiness <- function(meltedState, tolerance){
  ...
  meltedState <-meltedState %>% rowwise() %>% mutate(happy= isHappy(state = mat,X,Y,tolerance))
}
...
#we update the function to display sad smileys
...
unhappy <-  getUnhappy(stateEnv, 0.3)
